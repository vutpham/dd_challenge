...
class ChatRoomsList extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.selected === null && this.state.rooms.length > 0) selectChatroom(0);
  }

  displayChatrooms() {
    const {selected, selectChatroom} = this.props;
    return this.state.rooms.map(({name, id}) => <ChatRoomsListItem key={id} name={name} selectChatroom={() => selectChatroom(id)} isSelected={id === selected}/>)
  }
